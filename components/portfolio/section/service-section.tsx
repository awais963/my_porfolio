...
export  const  ServicesSection: React.FC<ServiceSectionProps> = ({ isDarkMode }) => {
  const [hoveredService, setHoveredService] = useState<string | null>(null)
const service = sectionContent.services
  return (
    <SectionWrapper id={service.id} title={service.title} description={service.description} fromColor={service.gradientFrom} toColor={service.gradientTo}  isDarkMode={isDarkMode} >
       {/* Services Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            ...
            >
              {/* Background Pattern */}
              <div className="absolute top-2 right-2 w-16 h-16 opacity-5 dark:opacity-3">
